...
def find_support_vectors(svm_reg):
    """Red marks the points that go beyond the support vectors"""
...
def plot_svm_regression(svm_reg_fun, axes_fun):
    """Displays set points and draws vectors"""

    support_vectors = find_support_vectors(svm_reg_fun)
    ...
    plt.plot(x1s, y_pred - epsilon, "k--", zorder=-2)
    plt.scatter(X[support_vectors], y[support_vectors], s=180, facecolors='red', zorder=-1)
    plt.plot(X, y, "bo")
    plt.plot(x1s, y_pred)
    plt.xlabel("$x_1$")
...
if __name__ == "__main__":
    ...
    plt.sca(axes[0])
    plot_svm_regression(svm_linear_reg, [-1.5, 1.5, -0.4, 1])
    plt.title(f"C={svm_linear_reg[-1].C}, "
              f"epsilon={svm_linear_reg[-1].epsilon}")
    plt.grid()

    plt.sca(axes[1])
    plot_svm_regression(svm_poly_reg, [-1.5, 1.5, -0.4, 1])
    ...
    plt.sca(axes[0])
    plot_svm_regression(svm_rbf_reg, [-1.5, 1.5, -0.4, 1])
    ...
    plt.sca(axes[1])
    plot_svm_regression(svm_sigmoid_reg, [-1.5, 1.5, -0.4, 1])
